...
| ~~`ContactController`/`TaskController::updateStatus` validam inline em vez de `FormRequest`~~ | — | Entrada duplicada e desatualizada — ver a entrada de mesmo tema acima, já marcada "Totalmente resolvido (2026-07-27)". Confirmado de novo em código (2026-07-30): `TaskController::updateStatus()` usa `UpdateTaskStatusRequest`, `PersonController::avatar()` usa `UpdateAvatarRequest`. Removida por ser ruído redundante no índice. |
